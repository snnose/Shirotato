...
public class BrushControl : MonoBehaviour, IMeleeWeaponControl
{
    ...
    private void Awake()
    {
        ...
        switch (weaponInfo.GetWeaponGrade())
        {
            case 1:
                waffleDropRate = 0.06f;
                break;
            case 2:
                waffleDropRate = 0.1f;
                break;
            case 3:
                waffleDropRate = 0.15f;
                break;
            default:
                waffleDropRate = 0.03f;
                break;
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
            ...
            isAttackPossible)
        {
            ...
            // 확률에 따라 와플을 드랍한다 (솔 특수 효과)
            TryDropWaffle(hitedMonster);
    ...
    // 확률에 따라 공격한 몬스터의 위치에 와플 드랍
    void TryDropWaffle(GameObject monster)
    {
        float random = Random.Range(0f, 1f);

        if (random < waffleDropRate)
        {
            GameObject copy = Instantiate(Resources.Load<GameObject>("Prefabs/Drops/Waffle"));
            copy.transform.position = monster.transform.position;
        }
    }
