...
public class BatControl : MonoBehaviour, IMeleeWeaponControl
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
            ...
            isAttackPossible)
        {
            ...
            // 방망이는 레벨 비례 추가 대미지가 있음
            float finalDamage = damage + LevelProportionDamage(weaponInfo.GetWeaponGrade());
    ...
    float LevelProportionDamage(int rarity)
    {
        int level = ExpManager.Instance.GetCurrentLevel();
        float additionalDamage = 0f;

        switch (rarity)
        {
            case 1:
                additionalDamage = level * 0.85f;
                break;
            case 2:
                additionalDamage = level * 1.0f;
                break;
            case 3:
                additionalDamage = level * 1.25f;
                break;
            default:
                additionalDamage = level * 0.75f;
                break;
        }
